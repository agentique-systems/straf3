//! **Criterion 4 on the geometry that matters**: the arena half.
//!
//! # The gap this closes
//!
...
//! runs the hardcoded arena with ramps (spec rev 6 §T option (a)), and
...
//! 2. **This file** — the arena, self-consistency: the same recorded input
...
//! world — so if the arena's digests are invariant under a schedule of 1 ms
...
use support::{Run, arena_runs, assert_digests_match, parse_trace_csv};
...
/// The arena replays identically under every frame schedule.
///
...
#[test]
fn the_arena_replays_identically_under_every_frame_schedule() {
    let schedules: [&[u64]; 6] = [
        &[1],
        &[7],
        &[9],
...
/// Replaying the same arena input twice must be bit-identical.
///
...
#[test]
fn replaying_the_same_arena_input_twice_is_bit_identical() {
    for run in arena_runs() {
...
/// The arena run must actually *meet the geometry*.
///
...
/// cheapest observable proof that the arena's collision world was consulted,
/// and it is asserted here rather than assumed because the fixture's spawn
/// point was chosen before the arena existed.
#[test]
fn the_arena_run_actually_touches_the_arenas_geometry() {
    for run in arena_runs() {
        let bin = straf3();
        ...
        let mut rows = stdout.lines().skip(1).filter(|l| !l.trim().is_empty());

        // Column 9 of the trace CSV is `grounded`.
        ...
        assert!(
            ...
            "{}: the run was never grounded across the whole replay — it never \
             touched the arena's geometry, so this fixture proves nothing about \
             ramps. Retune its spawn point and heading against the arena as \
             built.",
            run.name,
        );
    }
